...
exports.healthCheck = exports.placeDetails = exports.placesAutocomplete = void 0;
...
const validateUserAuth = (context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated to use Places API');
    }
};
/**
...
const checkRateLimit = async (userId) => {
    ...
    const requests = ((data === null || data === void 0 ? void 0 : data.requests) || []).filter((t) => t > oneHourAgo);
...
exports.placesAutocomplete = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (!data.input || typeof data.input !== 'string') {
            throw new functions.https.HttpsError('invalid-argument', 'Missing or invalid "input" parameter');
        }
        const input = sanitizeInput(data.input);
        if (input.length < 2) {
            throw new functions.https.HttpsError('invalid-argument', 'Input must be at least 2 characters');
        }
        // Check if API key is configured
        if (!GOOGLE_PLACES_API_KEY) {
            throw new functions.https.HttpsError('internal', 'API not configured. Please contact support.');
        }
        // Call Google Places API (server-side, key is protected)
        // 🔄 Use withRetry for automatic retry on network failures
        const response = await withRetry(() => axios_1.default.get('https://maps.googleapis.com/maps/api/place/autocomplete/json', {
            params: {
                input,
...
exports.placeDetails = functions.https.onCall(async (data, context) => {
    try {
        ...
        if (!data.place_id || typeof data.place_id !== 'string') {
            throw new functions.https.HttpsError('invalid-argument', 'Missing or invalid "place_id" parameter');
        }
        ...
        const response = await withRetry(() => axios_1.default.get('https://maps.googleapis.com/maps/api/place/details/json', {
            params: {
 ...
 */
exports.healthCheck = functions.https.onCall(async (_data, context) => {
    validateUserAuth(context);
    return {
        status: 'ok',
...
//# sourceMappingURL=placesProxyExample.js.map