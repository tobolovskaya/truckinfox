...
exports.sendBatchNotificationsOnNewRequest = functions.firestore
    ...
    try {
        ...
        console.log(`✅ Batch notification completed for request ${requestId}:`, `${totalSent}/${carrierTokens.length} delivered (${((totalSent / carrierTokens.length) *
            100).toFixed(1)}%)`);
    }
    catch (error) {
...
exports.retryFailedNotifications = functions.pubsub
    ...
    try {
        ...
        console.log(`✅ Retry completed: ${totalSuccessful}/${totalRetried} successful (${((totalSuccessful / totalRetried) *
            100).toFixed(1)}%)`);
        return null;
    }
    catch (error) {
